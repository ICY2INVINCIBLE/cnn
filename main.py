from read_train import gen_captcha_text_and_image_new, ALPHABET,number, alphabet
from read_test import get_test_captcha_text_and_image, get_test_sets_length
import numpy as np
import tensorflow as tf

text, image = gen_captcha_text_and_image_new()
print("验证码图像channel:", image.shape)  # (40, 120, 3)
# 图像大小
...
def convert2gray(img):
    if len(img.shape) > 2:
        ...
        # int gray = (int) (0.3 * r + 0.59 * g + 0.11 * b);#rgb比例
        return gray
    else:
        return img
...
FC_dim1 = float(IMAGE_HEIGHT) / (strides_pool1[1] * strides_pool2[1] * strides_pool3[1])
FC_dim2 = float(IMAGE_WIDTH) / (strides_pool1[2] * strides_pool2[2] * strides_pool3[2])
FC_dim = int(round(FC_dim1) * round(FC_dim2) * 64)
# Read_main定义CNN
def crack_captcha_cnn(w_alpha=0.01, b_alpha=0.1):
    ...
    # 随机生成权重
    w_d = tf.Variable(w_alpha * tf.random_normal([FC_dim, 1024]))
    # 随机生成偏置
